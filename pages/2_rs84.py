...
import rs84_core as core
import rs84_explain as ex
    ...
    with st.expander('Szindróma: $\\mathbf{s} = \\mathbf{r} \\, H^{\\mathsf{T}}$ — minden **$s_i$** ( $\\mathbf{r}$ és **$H$** soronkénti szorzata )', expanded=False):
        ...
        for i in range(nk):
            with syn_step_cols[i]:
                ...
                for ln in si_lines:
                    st.markdown(ln)
                st.success(f'**Összeg (s_{{{i}}})** (GF(16), **⊕**): {_syndrome_sum_markdown(si_sum, mask=15, alpha_map=INT_TO_ALPHA_STR)}')
...
def render_s0_expander(c_vec, H_mat) -> None:
    nk = int(H_mat.shape[0])
    with st.expander('Első szindróma-komponens: **c·Hᵀ = 0** — **s₀**, **s₁**, **s₂**, **s₃** (aktuális **c** és **H** sorai)', expanded=False):
...
st.title('RS(8,4) – hibajavító kódolás')
_rs_d_min = N - K + 1
_rs_t = (_rs_d_min - 1) // 2
st.markdown(
...
with st.expander('GF(16) elemek: karakter ↔ 4 bit ↔ polinom ↔ α hatvány', expanded=False):
    ...
    st.dataframe(core.gf16_element_table_rows(), use_container_width=True, hide_index=True)
...
with tab_syn:
    ...
    st.markdown(
        '**s** GF(16) int (0–15): `' + core.format_int_row(s_ints) + '`  \n**α hatvány alak** (s₀…s₃): '
